...
if Overflow then
    ...
    function Overflow.bulk_use(card, area, amount)
        ...
        for i = 1, #effects do
            for _, eff in pairs(effects[i]) do
                while eff do
        ...
        for _, eff in ipairs(retrig_effects) do
            if bulk_use_ref and eff.bulk_use_threshold and (eff.repetitions or 0) >= eff.bulk_use_threshold and Overflow.can_bulk_use(card) then
                if not eff.no_message then card_eval_status_text(eff.message_card or eff.card or card, "extra", nil, nil, nil, { message = eff.bulk_use_message or eff.message or localize("k_again_ex"), colour = eff.colour or G.C.FILTER }) end
                bulk_use_ref(card, area, eff.reptitions)
            else
...
function Card:use_consumeable(area, copier)
    ...
    for _, eff in ipairs(retrig_effects) do
        if bulk_use_ref and eff.bulk_use_threshold and (eff.repetitions or 0) >= eff.bulk_use_threshold and Overflow.can_bulk_use(self) then
            if not eff.no_message then card_eval_status_text(eff.message_card or eff.card or self, "extra", nil, nil, nil, { message = eff.bulk_use_message or eff.message or localize("k_again_ex"), colour = eff.colour or G.C.FILTER }) end
            bulk_use_ref(self, area, eff.reptitions)
        else
            for _ = 1, math.floor(eff.repetitions or 0) do
